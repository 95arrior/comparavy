...
/** ★오글 멘트 금지(유저 2026-07-14: "판매왕 양심에 걸고 정성으로만" 류 — 신뢰는 데이터가 만들지 선언이 아니다) */
export const CRINGE_PATTERNS = [/판매왕/, /양심에\s*걸고/, /정성(으로만|을\s*다해|을\s*담아)/, /약속드립니다/, /믿어\s*주세요/, /진심을\s*담아/];
/** 상품 이미지 자리 — 글마다 3~5곳 변동(유저: 3개 이상, 그때그때 다르게) */
export const PRODUCT_IMG_RANGE = { min: 3, max: 5 };

/** ★기능 단정 스캔(외부 검수 반영 2026-07-14) — 효능·기능을 사실처럼 단정하면 실격, "~에 도움이 될 수 있습니다"로 */
export const CLAIM_PATTERNS: { re: RegExp; fix: string }[] = [
  { re: /(냄새|악취)(를|가)\s*(완전히\s*)?(잡아|잡습|잡히|없애|제거해|제거합)/, fix: "냄새 완화에 도움이 될 수 있습니다" },
  { re: /곰팡이(를|가)\s*(완전히\s*)?(잡아|잡습|없애|제거해|제거합|지워|사라지)/, fix: "곰팡이 얼룩 제거에 도움이 될 수 있습니다(리뷰 인용 프레임 권장)" },
  { re: /살균(이|을)?\s*(된다|됩니다|해\s*줍니다|해준다|합니다)/, fix: "제품에서 제공하는 살균 기능(지원합니다 프레임)" },
  { re: /세균(을|이)\s*(잡|죽|박멸|없애)/, fix: "습한 환경은 세균과 냄새의 원인이 될 수 있습니다" },
  { re: /(무조건|반드시)?\s*(효과(가|를)\s*(있습니다|봅니다|확실))/, fix: "도움이 될 수 있습니다" },
  { re: /못\s*팝니다/, fix: "~라면 구매 전 이 부분을 꼭 고려해 보세요" },
];
