...
export async function writeArticle(p: Product, kw: KeywordResult, brief: PsychBrief, reviews: ReviewMining, angleIdx = 0): Promise<ArticleDraft> {
  ...
  const draft = await askJson<ArticleDraft>(
    [
      ...
      `4. 구매 전 체크포인트(★단점의 착한 풀이 — 2026-07-15 유저 확정: 사용도 안 해본 단점 단정은 악플다. 숨기는 게 아니라 풀어주는 것): 불만 TOP 1~2개를 다루되 반드시 3단 세트로 — ①한정: 누구에게/어떤 상황에서만 해당하는지("조용한 사무실 기준이라면") ②리뷰 속 대처·완화 맥락("30분 환기로 해결했다는 후기가 있었어요" — 리뷰에 실제 있는 것만) ③관점 전환: 그 특성의 이면 가치(가격 부담→1회 비용 환산·대용량, 향 강함→효과가 확실한 이면). ★단정 부정 문장 단독 금지("소음이 큽니다." 띡 — 금지). 마무리 화법은 "~에 민감한 분이라면 구매 전 이 부분을 확인해 보세요".`,
