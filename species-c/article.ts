...
export async function writeArticle(p: Product, kw: KeywordResult, brief: PsychBrief, reviews: ReviewMining): Promise<ArticleDraft> {
  ...
  const draft = await askJson<ArticleDraft>(
    [
      ...
      `2. 리뷰 집계: 위 신뢰 회계 표준 문장으로 열고 + 만족 TOP3. 인용은 나열형이 자연스럽다: "'보송보송' '냄새가 줄었다' 같은 의견이 반복되었습니다"(구체 표현 2~3개 나열, 각 15자 이내). 이 섹션 끝에 단독 줄로 두 번째 ${MARKER_PRODUCT_IMG} (상품을 한 번 더 보여줄 자리).`,
      ...
      `5. CTA 1: 구매 제안 2~3문장 — ★마지막 문장은 바로 아래 링크 카드를 받는 완곡한 안내("가격과 상세 스펙은 아래 링크에서 확인할 수 있습니다" 톤 — 조급한 재촉 금지). 끝에 단독 줄: ${MARKER_LINK_1}`,
