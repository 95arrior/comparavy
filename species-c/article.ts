...
export async function writeArticle(p: Product, kw: KeywordResult, brief: PsychBrief, reviews: ReviewMining, angleIdx = 0): Promise<ArticleDraft> {
  ...
  const draft = await askJson<ArticleDraft>(
    [
      ...
      `1. 문제 공감 도입(★총 4문장 이내 — 유저: 서론 나열 금지): ①독자 속마음 따옴표 대사 1줄(30자 이내 — 리뷰 인용과 별개 허용) ②상황 1문장 ③문제 선언 1문장 ④★이득 한 줄(교본 흡수): 이 글에서 얻는 것을 구체로 — 할인 정보가 있으면 그것(${p.discountPct ? `현재 ${p.discountPct}% 할인 중` : "할인 정보 없음 — 지어내지 마"}), 없으면 해결 약속 한 줄. 메인 키워드 자연 포함. 도입 끝에 단독 줄로: ${MARKER_PRODUCT_IMG}`,
      `1.5. ★결론 블록(구매 독자용 — 도입 직후 3초 안에 답을 준다): "결론: " 으로 시작하는 1~2문장 — 무엇이 답인지 + 가격(할인 중이면 함께) + 누구에게 맞는지. 예: "결론: 세탁조 냄새의 원인 제거엔 이거 하나면 됩니다. 지금 30% 할인가 25,300원이고, 일주일에 한 번 넣기만 하면 되는 방식이에요." 서스펜스로 결론을 미루지 마라 — 결론을 먼저 준 글이 끝까지 읽힌다.`,
      ...
      `2. 리뷰 집계: 위 신뢰 회계 표준 문장으로 열고 + 만족 TOP3. 인용은 나열형이 자연스럽다: "'보송보송' '냄새가 줄었다' 같은 의견이 반복되었습니다"(구체 표현 2~3개 나열, 각 15자 이내). ★리뷰→해석(화법 v3 — 관찰 보고 금지): "~의견이 많았습니다/~후기가 자주 보였습니다" 같은 보고서 어미는 글 전체 1회까지만. 각 포인트는 관찰을 짧게 언급("가장 많이 언급된 건 '젤이라 흘러내리지 않는다'는 점") → 곧바로 내 목소리 해석("세로면 실리콘엔 액상보다 젤이 편하다는 얘기죠", "~와 연결될 수 있어 보여요!"). 제형·성분·구조 같은 일반 상식 해석은 내 문장으로 자신 있게 말한다(지식이지 사용 경험이 아니다). ★이 섹션 끝에 단독 줄로: ${MARKER_LINK_1} (리뷰 신뢰가 쌓인 직후가 첫 링크 자리 — 앞 문장이 "지금까지 리뷰가 궁금하게 했다면 아래에서 실물을 확인할 수 있어요" 류로 링크 카드를 받는다).`,
      ...
      `4. 구매 전 체크포인트(★단점은 유하게+해결 즉시 — 2026-07-15 유저 최종: 단점으로 구매 실패를 유도하면 안 된다): 불만 1~2개를 다루되 ★한 호흡 규칙 — 단점 언급과 해결이 같은 문장 또는 바로 다음 문장에 붙어야 한다("작동음이 있다는 후기가 있었는데, 30분 환기면 충분했다는 팁이 바로 따라붙었어요"). 표현 강도는 최약체로: '크다·불편하다·아쉽다' 대신 '~한 편', '~할 수 있어요'. ②관점 전환으로 닫기: 그 특성의 이면 가치(가격 부담→1회 환산하면 커피 한 잔·대용량, 향 강함→효과가 확실하다는 신호). ③마무리는 걸러내기가 아니라 안심: "이 부분만 알고 쓰면 신경 쓸 일은 아니라는 게 리뷰들의 결이었어요" 톤. ★단정 부정 단독 문장 금지, 해결 없는 단점 언급 금지.`,
