...
function Chip({ item }: { item: Kw }) {
  return item.ssak ? (
    <span className="ateflo-chip-aurora mx-1 inline-flex shrink-0 items-center whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold text-[#3f3a6b] shadow-sm ring-1 ring-white/50">
      {item.t}
    </span>
  ) : (
    <span className="mx-1 inline-flex shrink-0 items-center whitespace-nowrap rounded-full bg-neutral-100 px-4 py-2 text-sm font-medium text-neutral-500 ring-1 ring-black/[0.03]">
      {item.t}
    </span>
  );
}
