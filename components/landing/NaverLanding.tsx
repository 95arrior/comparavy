      ...
      <div style={{ opacity: fade, transform: `translateY(${lift}px)` }}>
        ...
        <Rise delay={180}>
          <p className="mt-6 max-w-md text-[16px] leading-relaxed text-neutral-500">
            뭘 쓸지 고민하지 마세요. 아침마다 완성된 글이 도착해 있어요 — 읽고, 붙여넣으면 발행 끝. 발행 확인까지 자동이에요.
          </p>
        </Rise>
        <Rise delay={260}>
...
const SCENES = [
  ...
  { k: "write", label: "02 — 집필", title: "아침이면 이미 완성돼 있어요", body: "홈에 들어온 순간 오늘의 글이 미리 써지기 시작해요. 체크인 한 번 하고 나면 기다림 0초 — 읽고 다듬기만 하면 돼요. 네이버 로직(C-Rank·D.I.A.) 규격, 계정마다 다른 문체로." },
  { k: "done", label: "03 — 발행", title: "붙여넣으면 끝, 확인은 자동", body: "자동 발행은 계정을 위험하게 해요. 마지막 붙여넣기만 남기는 게 우리 방식 — 발행되면 블로그 RSS로 자동 확인하고, 게이지가 올라가요." },
];
      ...
      <section className="mx-auto max-w-3xl px-5 pb-28">
        ...
        <div className="mt-9 space-y-3">
          {[
            ...
            { t: "발행 다음도 코스예요", s: "애드포스트 승인 → 쇼핑커넥트 → 체험단 → 인플루언서. 지금 할 것 하나만 열어주는 수익화 사다리로 안내해요." },
            { t: "터진 글은 시리즈로 이어가요", s: "반응이 온 글은 다음 날 후속 글감으로, 큰 주제는 3~4편 시리즈로 — 트래픽이 한 편에서 끝나지 않게 해요." },
      ...
      <section className="mx-auto max-w-2xl px-5 pb-28">
        ...
        <div className="mt-9 space-y-2.5">
          {[
            ...
            { q: "블로그 여러 개도 되나요?", a: "네, 5개까지. 블로그마다 주제·문체·글감이 완전히 분리되고, 크레딧은 하나의 지갑을 같이 써요." },
          ].map((f, i) => (
