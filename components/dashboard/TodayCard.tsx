...
export default function TodayCard({
  ...
  topic: { keyword: string; title: string; tag?: string; newsContext?: string; briefText?: string } | null;
  loading: boolean;
  credits: number;
  info: CourseInfo;
  ...
  if (info.publishedToday) {
    return (
      <Card>
        ...
        {topic && !locked && (
          <button onClick={() => onWriteKeyword(topic.keyword, topic.title, topic.newsContext, topic.briefText)} className="mt-3 w-full rounded-xl bg-neutral-100 py-2.5 text-[13px] font-bold text-neutral-600 transition hover:bg-neutral-200 active:scale-[0.99]">
  ...
  if (locked) {
    return (
      <Card highlight>
        ...
        <button onClick={() => onWriteKeyword(topic.keyword, topic.title, topic.newsContext, topic.briefText)} className="mt-3.5 w-full rounded-xl bg-[#1D75F7] py-3.5 text-[15px] font-bold text-white transition hover:opacity-90 active:scale-[0.99]">
          코스 이어가기
        </button>
      </Card>
    );
