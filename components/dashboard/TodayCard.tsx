  ...
  if (info.publishedToday) {
    ...
    return (
      <Card plain={plain}>
        ...
        {!topic && !locked && (
          <div className="mt-3 flex items-center gap-2.5 rounded-[12px] bg-[#F7F8FA] px-4 py-3">
            <span className="tk-wand text-[14px] text-[color:var(--color-brand)]" aria-hidden>✦</span>
            <span className="text-[13px] text-[color:var(--color-text-sub)]">새 글감을 모으고 있어요 — 찾으면 여기 자동으로 나타나요</span>
          </div>
        )}
        {topic && !locked && (
