...
export default function WpHome({ blogName, blogId, articles, credits, onOpenArticle, onAddBlog }: {
  ...
  const SETUP_ITEMS = [
    ...
    { k: "naver", t: "네이버에도 내 블로그 등록 (한국 검색의 절반)", steps: [
      ...
      "소유 확인은 ‘HTML 태그’ 방법 — 구글 서치콘솔 때와 같은 방식이에요",
      "확인되면 ‘요청 → 사이트맵 제출’에 sitemap.xml 이라고 적고 제출해요",
      "— 워드프레스 글도 네이버 검색에 떠요. 우리 글감은 네이버 검색 수요로도 검증된 키워드라, 등록만 하면 네이버 유입까지 이중으로 받아요",
    ] },
