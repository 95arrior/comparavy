...
export default function Home({
  ...
  const topicsCacheKey = () => `ateflo_topics_v20_${todayDate}_${profileKey ?? ""}_normal`;
  ...
  const loadTopics = useCallback(async () => {
    const ck = `ateflo_topics_v20_${new Date().toISOString().slice(0, 10)}_${profileKey ?? ""}_normal`;
    try {
