...
interface Topic { keyword: string; title: string; demandLabel: string; vol: number; comp: Comp; tag?: string; blogTotal?: number | null; newsContext?: string; briefText?: string }
...
export default function Home({
  ...
  credits: number;
  onWriteKeyword: (keyword: string, title: string, newsContext?: string, briefText?: string) => void;
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 pb-10">
      ...
      <div className="at-rise at-d2 mt-6">
        <TodayCard
          topic={first ? { keyword: first.keyword, title: first.title, tag: first.tag, newsContext: first.newsContext, briefText: first.briefText } : null}
          ...
          <div ref={moreRef} className="at-rise mt-2 scroll-mb-24">
            {topicsLoading ? (
              ...
              <div className="flex flex-col gap-2.5">
                {rest.map((t) => (
                  <TopicRow key={t.keyword} topic={t} onClick={() => onWriteKeyword(t.keyword, t.title, t.newsContext, t.briefText)} onSwap={() => swapTopic(t.keyword)} swapping={swapping.includes(t.keyword)} />
                ))}
              </div>
            ) : (
...
function TopicRow({ topic, onClick, onSwap, swapping }: {
  topic: { keyword: string; title: string; vol: number; comp: Comp; blogTotal?: number | null; tag?: string; newsContext?: string; briefText?: string };
  onClick: () => void;
  onSwap?: () => void;
  swapping?: boolean;
}) {
