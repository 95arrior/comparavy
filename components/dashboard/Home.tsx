...
export default function Home({
  ...
  const topicsCacheKey = (md: string) => `ateflo_topics_v9_${todayDate}_${profileKey ?? ""}_${md}`;
  ...
  const loadTopics = useCallback(async () => {
    ...
    const ck = `ateflo_topics_v9_${new Date().toISOString().slice(0, 10)}_${profileKey ?? ""}_${md}`;
