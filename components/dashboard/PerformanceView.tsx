...
function buildPaths(pub: number, onWrite: () => void, approved = false): { title: string; paths: Path[]; otherChannels: Path[] } {
  ...
  return {
    ...
    paths: [
      ...
      { label: "네이버 쇼핑커넥트", desc: "상품 추천 수수료 · 네이버 자체 제휴", conditions: `애드포스트와 별개로 신청할 수 있어요(개설 90일 대기 없음 — 2026.7 실측). 수수료는 판매자 설정에 따라 상품별로 달라요. ${ASOF}`, how: [
        "brandconnect.naver.com 접속 → 크리에이터로 시작 → 내 블로그 채널을 연동해요.",
        "활동 주제: 메인 = 내 블로그 주제(예: 경제/비즈니스), 서브 2개 = 내 글 톤으로 리뷰 쓸 수 있는 것(예: 리빙·IT테크). 30일간 변경 불가라 신중히.",
        "상품 카테고리(최대 3): '내 블로그 프레임으로 소화 가능한 상품군'으로 — 예: 절약 프레임이면 생활가전·주방가전·생활용품.",
        "공동구매 경험 질문: 없으면 '없음' 솔직하게 + 비즈니스 이메일은 입력 추천(브랜드 직접 제안 통로).",
        "승인 후: 캠페인 상품을 골라 제휴 링크 생성 → 리뷰·비교 글의 [상품 링크 자리]에 넣으면 끝.",
      ], tips: ["'커미션 기회' 칩이 붙은 글감이 이 링크와 궁합이 좋아요.", "협찬·제휴 글 비중은 주간 발행의 3할 이하로 — 정보 글이 본체여야 블로그가 오래가요.", "'열렸어요'는 기회가 열린 거지 수익 보장이 아니에요."], status: "now", note: "", emoji: "cart", cta: { label: "쇼핑커넥트 신청하기", url: "https://brandconnect.naver.com" } },
