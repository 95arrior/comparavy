...
export async function POST(request: Request) {
  ...
  const prompt = [
    ...
    `네이버 블로그 썸네일에 큰 글씨로 박을 문구 6개를 만들어줘. ★대전제: 문구는 글 제목의 핵심을 임팩트 있게 압축한 것이다. 제목을 읽지 않아도 무슨 글인지 알아야 하고, 팩트만 담는다(수식·감상 금지). 절대 조건: 각 문구는 공백 포함 18자 이내(고정 폰트 — 한 줄 9자 x 2줄), 짧을수록 강하다(6~14자 지향). 글 제목: "${art.title}" / 키워드: "${art.keyword}"${art.meta_description ? ` / 요지: ${String(art.meta_description).slice(0, 150)}` : ""}`,
    "- ★구조: 윗줄은 '무엇에 대한 글인지'(제목의 핵심 명사구를 그대로 써도 된다), 아랫줄은 '왜 봐야 하는지'(팩트 한 방). 예 — '중고 전기트럭도 / 보조금 받나요?', '청년 창업 / 모르면 나만 손해', '집 몇 채냐로 / 세금이 0원 될까'.",
    "- ★팩트만: 본문·제목에 실제로 있는 사실·숫자·조건만 쓴다. '대박·필수·완벽정리' 같은 수식과 감상은 실격. 없는 숫자를 만들지 마라.",
    "- ★제목을 통째로 옮기지 마라 — 제목보다 짧고 세야 한다. 제목의 어절을 그대로 쓰는 건 괜찮지만, 제목 전체를 줄만 바꿔 옮긴 것은 실격.",
