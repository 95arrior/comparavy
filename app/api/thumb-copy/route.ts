...
export async function POST(request: Request) {
  ...
  const prompt = [
    ...
    `네이버 블로그 썸네일에 큰 글씨로 박을 문구 6개를 만들어줘. ★대전제: 문구는 글이 말하는 것 중 '가장 센 사실 하나'를 한 방으로 던지는 것이다. 제목을 요약하는 게 아니다 — 제목이 나열한 것 중 독자가 가장 놀랄 사실을 골라 숫자와 함께 박는다. 팩트만 담는다(수식·감상 금지). 절대 조건: 각 문구는 공백 포함 18자 이내(고정 폰트 — 한 줄 9자 x 2줄), 짧을수록 강하다(4~10자 지향 — 2026-08-17 유저 재확정, 감정·질문 위주: '딱 1세대'·'월급 또 줄어?'). ★제목이 이미 설명하고 있다 — 문구는 설명 요약이 아니라 사건의 가장 뾰족한 조각 하나다: '딱 1세대'·'8월 18일 접수'·'월 7만원 차이'처럼 2~8자 초단문이 1순위. 글 제목: "${art.title}" / 키워드: "${art.keyword}"${art.meta_description ? ` / 요지: ${String(art.meta_description).slice(0, 150)}` : ""}`,
