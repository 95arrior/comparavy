...
export async function POST() {
  ...
  try {
    ...
    try { body = insertBanners(body, await generateWpBannersToStorage(user.id, pick.keyword, `${b.id}-${kstDay()}-${crypto.randomUUID().slice(0, 8)}`, 3, String((b as { blog_name?: string | null }).blog_name ?? "")), pick.keyword); } catch { /* ignore */ }
    const ins = {
