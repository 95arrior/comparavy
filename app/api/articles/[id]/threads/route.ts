...
// 블로그 글 → 스레드(Threads) 최적화 게시물(~250자, 정보 글 맥락 유지, 해시태그 없음). 원칙을 프롬프트로 박는다.
      ...
      messages: [
        {
          ...
          content:
            ...
            `2) 길이: 정보 글이라 너무 짧으면 맥락이 잘린다. 200~320자 권장(공백 포함, 최대 400자). 앞·뒤 맥락은 살리되 중복·군더더기만 덜어내라.\n` +
            `3) 문장은 짧게, 빈 줄로 자주 끊어 읽기 쉽게(스캔만 해도 읽히게).\n` +
            `4) 다짜고짜 홍보 금지. 잠재 고객에게 '도움되는 정보'로 신뢰를 쌓는다.\n` +
            `5) 마지막 줄은 대화를 부르는 한 마디(질문/공감 유도).\n` +
            `6) 과장·보장·최고·1위·100% 같은 단정 금지. 거짓 정보 금지.\n` +
            `7) 출력은 '스레드에 그대로 붙여넣을 순수 텍스트'만(설명·따옴표·마크다운·머리말 없이). 해시태그는 넣지 마라(요즘 스레드엔 해시태그 없이가 자연스럽다).`,
        },
      ],
    });
