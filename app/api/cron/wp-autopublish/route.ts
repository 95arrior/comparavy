  ...
  for (const b of blogs) {
    try {
      ...
      try {
        ...
        try { body = insertBanners(body, await generateWpBannersToStorage(b.user_id, pick.keyword, `${b.id}-${kstDay()}`, 3, String((b as { blog_name?: string | null }).blog_name ?? "")), pick.keyword); } catch { /* 배너 실패 — 계속 */ }
        const ins = {
