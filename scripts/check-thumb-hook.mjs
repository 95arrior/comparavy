...
//  유저가 준 레퍼런스 썸네일이 전부 제목의 핵심을 그대로 쓰고 있었다 — ★실물이 규칙을 이긴다.
{
  ...
  // ★유저가 준 정답 한 줄이 규격을 정한다(2026-08-06):
  //  제목 "신혼부부 매매대출 조건·한도·신청 절차 완전 정리" → 썸네일 "아는 부부만 3.2억까지 받는다"
  //  ★제목 어절을 하나도 안 가져왔다. 구체 숫자 + 결핍 훅이다.
  // ★2026-08-17 유저 재반전("전부 '아니다' 계열 — 하나의 감정을 4번 변형했다"): 8/6 '제목 핵심 그대로' 규격 폐기.
  //  새 규격: 훅 패밀리 6종 강제 + MONEY/TIME 분기 + 채점 배점(팩트 20/25 미만 즉시 탈락) + 제목은 요약하지 않는다.
  ok(/훅 패밀리 6종/.test(tc) && /A\. 반전형/.test(tc) && /F\. 대상·체감형/.test(tc), "★훅 패밀리 A~F — 후보 6개가 서로 다른 의미여야");
  ok(/MONEY 훅 vs TIME 훅/.test(tc), "★'내 돈' 기계 삽입 금지 — 금액 글만 MONEY, 과정·시점 글은 TIME");
  ok(/모바일 멈춤력 25/.test(tc) && /20점 미만이면 총점과 무관하게 즉시 탈락/.test(tc), "★채점 배점+팩트 일치 하드 컷");
  ok(/제목이 그 답이 된다/.test(tc), "★썸네일→'왜?'→제목이 답(요약 금지)");
  ok(/FACT_RISK/.test(tc) && /factRiskFail/.test(tc), "★공포형 어법 코드 게이트(날아간다류 — 본문 근거 없으면 탈락)");
  ok(/7~14자 전후가 최적/.test(tc), "★길이 7~14 지향(인물 썸네일과 경쟁 방지)");
  ok(/cn\.length >= tn\.length \* 0\.6/.test(tc), "★제목 통째 복사 실격(문턱 0.6 강화)");
