...
//  유저가 준 레퍼런스 썸네일이 전부 제목의 핵심을 그대로 쓰고 있었다 — ★실물이 규칙을 이긴다.
{
  ...
  ok(/가장 센 사실 하나/.test(tc), "★문구는 제목 요약이 아니라 '가장 센 사실 하나'");
  ok(/아는 부부만 3\.2억까지 받는다/.test(tc), "★유저의 정답 예시가 프롬프트에 박혀 있다");
  ok(/\[구체 숫자·조건 한 개\] \+ \[결핍·반전 한 마디\]/.test(tc), "★공식이 명시됐다");
  ok(/목차형 표현은 썸네일에서 실격/.test(tc), "★'조건·한도·절차'류 나열 금지");
