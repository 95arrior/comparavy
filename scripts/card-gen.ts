...
async function main() {
  ...
  let card = await generateCardNews(topic, angleLabel, { useWebSearch: angle?.web });
  let check = assessCard(card);
  if (!check.ok) {
    console.log(`  ↻ 품질 미달(${check.reasons.join(", ")}) → 재생성`);
    card = await generateCardNews(topic, angleLabel, { useWebSearch: angle?.web });
  ...
  console.log("④ 보관함에 적재…");
  const row = { type: "carousel", media_urls: urls, caption: card.caption, topic, threads_text: card.threadsText };
