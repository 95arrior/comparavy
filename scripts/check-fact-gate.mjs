...
// ── ★주제 오인 방지(2026-08-03 유저 실물에서 검거) ─────────────────────
//  '노무상담' 글이 사실검사 4건을 받았다 — 건강보험 임의계속가입·연차사용촉진·IRP 과세이연·분류과세.
//  상담 타이밍을 다루는 글이라 '퇴직 직후'·'퇴직서'·'퇴직금'이 자연스럽게 5번 나왔을 뿐인데
//  본문 3회 문턱에 걸려 퇴사 정산 글로 오인됐다. ★단어 출현을 주제로 착각한 것 —
//  위 예금 오탐과 같은 종류다. 소제목을 보면 갈린다: 진짜 주제면 h2에 선다.
{
  const chk = (c, label, extra = "") => { if (!c) fail++; console.log(c ? "OK " : "FAIL", "|", label, extra); };
  const 노무상담 = `<h2>무료 노무상담, 어디서 받나요?</h2><p>퇴직 직후, 혹은 이미 해고된 뒤에야 처음으로 노무상담을 검색합니다.</p>
<h2>유료 노무상담 비용이 얼마인가요?</h2><p>이미 퇴직서에 사인한 뒤라면 선택지가 줄어듭니다.</p>
<h2>상담 전에 꼭 챙겨야 할 것은?</h2><p>퇴직금·미지급 임금 문제라면 급여 내역이 유리합니다. 퇴사 전에 챙겨야 할 항목입니다.</p>
<h2>노무상담이 진짜 필요한 순간은?</h2><p>합의서나 퇴직서를 받았다면 서명 전이 골든타임입니다.</p>`;
  const miss1 = scanFacts(노무상담, "노무상담").filter((i) => i.layer === "missing");
  chk(miss1.length === 0, "★'노무상담' 글에 퇴사 정산 필수항목을 요구하지 않는다", `${miss1.length}건`);

  // ★과교정 방어 — 진짜 퇴사 정산 글에서는 그대로 잡아야 한다(소제목에 주제가 선다)
  const 퇴사정산 = `<h2>퇴직금 언제 들어오나요</h2><p>퇴사 후 14일 이내 지급됩니다.</p><h2>퇴사 후 건강보험은</h2><p>지역가입자로 전환됩니다.</p>`;
  chk(scanFacts(퇴사정산, "퇴직금 지급일").filter((i) => i.layer === "missing").length > 0, "★진짜 퇴사 정산 글에서는 여전히 누락을 잡는다");

  // 키워드엔 없어도 소제목이 주제를 말하면 인정
  const 소제목만 = `<h2>퇴사 후 건강보험 어떻게 되나</h2><p>정리했습니다.</p>`;
  chk(scanFacts(소제목만, "직장인 상식").filter((i) => i.layer === "missing").length > 0, "★소제목이 주제를 말하면 인정한다");
}
