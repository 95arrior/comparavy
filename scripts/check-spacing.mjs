...
// ★2026-07-14 여백 다이어트(유저: "빽빽함")로 기본 여백이 1→2칸이 됐다. 테스트가 옛 값에 머물러 FAIL이었다.
ok(gap(rich,"문단 하나\\.</p>","<p [^>]*>문단 둘")===2,"문단 사이 2");
ok(gap(rich,"문단 둘\\.</p>","<h2")===3,"소제목 앞 3");
ok(gap(rich,"</h2>","<p [^>]*>답 문단")===2,"소제목 뒤 2");
// ★긴 블록 아래 3→2(코드 주석: "실측: 과다"). 일반 문단이 이미 2칸이라 3칸은 뚝 끊겼다.
ok(gap(rich,"확실히 길죠</p>","<p [^>]*>짧은")===2,"긴 블록(4줄+) 아래 2");
ok(gap(rich,"짧은 문단\\.</p>","<p [^>]*><b>")===2,"강조 문장 앞 2");
