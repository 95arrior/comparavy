...
// ★기대값을 '정확한 분절'로 고정하지 않는다(2026-08-07 실측: 자동완성이 주는 분절이
//  "신용카드발급 신용점수" → "신용카드 발급 신용점수"로 그새 바뀌어 테스트만 깨졌다).
//  지켜야 할 불변식은 둘이다: ①글자는 그대로다(붙였다 뗐을 때 원본과 같다) ②띄어쓰기가 생겼다.
for (const raw of ["신용카드발급신용점수", "전세보증금반환확약서"]) {
  const got = await naturalizeKeyword(raw);
  ok(got.replace(/\s/g, "") === raw, "★글자 보존(띄우기만 한다)", `${raw} → ${got}`);
  ok(/\s/.test(got), "★사람 표기로 복원(띄어쓰기 발생)", `→ ${got}`);
}
