...
//  노출·회유 장치만 잃는다. 즉 없을 이유가 전혀 없다.
{
  ...
  // ★바이트 동일이 아니라 '태그 내용이 그대로인가'로 본다(2026-08-06).
  //  마감이 태그 줄을 걷어내고 맨 끝에 다시 놓기 때문에 개행 위치는 달라질 수 있다 —
  //  지켜야 할 것은 '모델이 쓴 태그를 덮지 않는다'는 것이지 문자열이 같다는 게 아니다.
  const tagsOf = (h) => (h.match(/#[가-힣A-Za-z0-9_]{2,}/g) ?? []).join(" ");
  ok(tagsOf(ensureHashtags(있음, "무기명채권")) === tagsOf(있음), "★이미 있으면 태그를 그대로 둔다", tagsOf(ensureHashtags(있음, "무기명채권")));
  ...
  const 진짜후 = ensureHashtags(진짜, "주식 리딩방", "재테크", 모델태그);
  const tagsOf2 = (h) => (h.match(/#[가-힣A-Za-z0-9_]{2,}/g) ?? []).join(" ");
  ok(tagsOf2(진짜후) === tagsOf2(진짜), "★진짜 해시태그가 있으면 덧붙이지 않는다(중복 방지)", tagsOf2(진짜후));
  ok(/<p>(?:\s*#[가-힣A-Za-z0-9_]{2,})+\s*<\/p>\s*$/.test(진짜후), "★태그 줄은 글의 마지막에 놓인다");
}
