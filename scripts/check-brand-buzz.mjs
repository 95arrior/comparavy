...
console.log("\n② 걸러야 하는 것 / 들여야 하는 것(규칙 재현):");
{
  ...
  const BLOCK = new RegExp(/(무직자|작업대출|신용불량|연체자|회생|파산|일수|사채|급전|당일대출|한도조회|신용점수\s?올리기|급등주|추천주|매수\s?타이밍|수익률\s?보장|리딩방)/);
  ...
  // ★차단은 '주제'가 아니라 '유인'을 막는다(2026-08-05 조정)
  ok(!BLOCK.test("전세자금대출 조건"), "★정상 소재(전세자금대출)는 이제 통과한다 — 통짜 차단이 폭을 죽이고 있었다");
  ok(BLOCK.test("무직자 작업대출"), "★불법사금융 유인은 그대로 차단");
  ok(BLOCK.test("급등주 추천"), "★투자권유 오인도 차단(주식 축을 열면서 같이 막는다)");
...
{
  const src = fs.readFileSync(new URL("../lib/brandBuzz.ts", import.meta.url), "utf-8");
  // ★유저 2026-08-05: "주식도 포함, 놓친 카테고리도 최대한 제한 두지 말고 관심사 높은 건 다 가져와요"
  for (const axis of ["청약", "분양", "전세", "재건축", "지원금", "환급금", "지역화폐", "청년",
                      "주식", "공모주", "배당금", "ETF", "ISA", "적금", "파킹통장",
                      "국민연금", "퇴직연금", "연금저축", "연말정산", "종합소득세", "재산세",
                      "취득세", "양도소득세", "전기요금", "통신비", "실손보험", "실업급여", "부업", "앱테크"]) {
    ok(new RegExp(`seed: "${axis}"`).test(src), `축 포함: ${axis}`);
  }
  ok((src.match(/\{ seed: "/g) ?? []).length >= 30, "★축이 30개 이상 — 폭을 좁게 잡으면 그 밖은 영원히 안 들어온다");
  ok(/const slot = Math\.floor/.test(src) && /all\.slice\(off\)/.test(src), "★축 회전 — 고정 순서면 앞쪽만 영원히 수확된다");
