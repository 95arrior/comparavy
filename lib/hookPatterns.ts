...
export const HOOK_PATTERNS: HookPattern[] = [
  ...
  // ★인용구형(2026-07-10 유저 채택 — 뉴스 1면 문법 “발화”…키워드, 미완결): 발화가 문을 열고 정보 공백이 클릭을 만든다
  { key: "quote", name: "인용구형", guide: "제목을 짧은 따옴표 발화로 연다 — “발화”…키워드, 미완결어 구조(뉴스 1면 문법). 발화는 검색자 누구나 속으로 하는 말(놀람·후회·발견)로 4~8자: “나만 몰랐나”, “이게 되네”, “벌써 마감?”. ★실존 인물·기관의 발언을 지어내지 않는다(씨앗 자료에 실제 발언이 있으면 그건 허용). 발화 뒤에는 키워드가 반드시 오고, 끝은 미완결 명사로 닫는다.", example: "“나만 몰랐나”…통장 이자, 두 배 차이 나는 이유" },
];
...
const BANNED_PATTERNS: RegExp[] = [
  ...
  /충격|경악|헉/,          // 유튜브식 어그로
  // ★감정 과잉 어휘(2026-07-17 PTRP 리서치) — 유튜브 16,215편 실측(Cui et al. 2024, JBR): 썸네일 '이미지'의 강한 감정은
  //  조회를 올리지만 '텍스트'에 실은 강한 감정은 정반대로 낮춘다. 우리는 텍스트 조판 카드 — 절제된 정보형 카피가 규격.
  //  낚시 프레이밍의 신뢰 훼손(Mukherjee 2022·Kaushal 2021)도 같은 방향. 손실 프레임('그대로 두면 손해')은 감정 어휘가 아니라 허용.
  /미쳤|미친/,
  /소름/,
  /실화/,
  /멘붕/,
