...
export function buildBodyPrompt(slotDesc: string, articleTitle: string, seed: number, opts?: { context?: string }): string {
  ...
  if (!useEmoji3d) { // 기본=실사 다큐
    ...
    return [
      ...
      "OVERRIDE RULE: if the scene hint describes objects on a desk/table (calculator, documents, keys, house model, coins) OR anything about phone/laptop/app SCREENS (잔고 확인·매수 화면·앱 화면·캡처), DISCARD that hint completely — it produces cheap stock-prop photos. Derive instead the real PLACE or human MOMENT behind the topic (loan topic → bank counter or apartment complex seen from below; moving topic → moving boxes moment). SIMPLIFY: reduce the scene hint to its ONE strongest visual anchor — a building, a counter, a place, or an object — and photograph that simply and beautifully. Ignore procedural details, screen contents, document items or written clauses entirely (예: '등기소 화면에서 근저당 확인' → 법원 건물 외관, '주민센터 창구에서 전입신고' → 창구 풍경만).",
