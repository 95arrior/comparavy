...
export const guideTypeStandards: Record<GuideLayoutType, string[]> = {
  "how-to": [
    ...
    "Preview concepts must support the public scroll-focus treatment: subtle scale, opacity, and translate animation on entry, with reduced-motion users seeing the final state immediately.",
...
export const comparavyGoldStandardPrompt = [
  ...
  "Every shortcut must include a realistic result preview concept near the top: task-matched preview type, clear input context when useful, realistic output, one short why-it-works explanation, no fake testimonials, no fake user stories, no unsupported performance claims, mobile-readable preview content, and scroll-focus animation support with reduced-motion fallback.",
