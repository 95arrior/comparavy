...
const REGION_RE = /([가-힣]{1,4}(?:특별시|광역시|특별자치시|특별자치도)|(?<![가친기도입출지연온햇])구(?![가-힣])|(?<![장군상])군(?![가-힣])|(?<![일당즉잠역표감과다행조실])시(?![가-힣])|경기도|강원도|충청[남북]도|전라[남북]도|경상[남북]도|제주도|경[남북](?![가-힣])|충[남북](?![가-힣])|전[남북](?![가-힣])|\[\s*(?:서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충남|전북|전남|경북|경남|제주)[^\]]*\])/;
