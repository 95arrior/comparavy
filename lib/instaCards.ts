...
export async function articleToInstaCards(title: string, bodyHtml: string, keyword: string, userId?: string | null): Promise<InstaPack | null> {
  ...
  const res = await client.messages.create({
    ...
    messages: [{
      ...
      content: [
        ...
        "  oneTake = ★단일 대본 하나만(2026-08-11 유저 최종: '20초 이하면 전부 OK, 초는 코드가 계산해 표시'): 상한 290자(초당 14자 실측 × 20초). 짧아도 된다 — 억지로 늘리지 마라. ★구조 = 사건→소개→팩트→유입, 뉴스 팩트체크식으로 짧게: ①실명 훅 1문장(사건) ②한 줄 소개 ③팩트 속사포 — 짧은 문장으로 증거·숫자 쌓기('사원증 카드키, GPS, 법인차량 기록. 다 걸렸어. 1~27분 만에 퇴근. 6개월 반복.') ④절정 직전 끊기+'자세한 내용은 아래를 확인해!'(유입). 같은 화자·반말·뒷담화 톤.",
        "",
  ...
  async function fitOneTake(current: string): Promise<string> {
    ...
    if (len <= 300) return current; // ★짧은 건 그대로(유저: '20초 이하면 전부 OK') — 넘칠 때만 줄인다
    try {
      const fix = await client.messages.create({
        model: "claude-haiku-4-5",
        max_tokens: 600,
        messages: [{ role: "user", content: [
          `아래 클립 대사를 290자 이하(공백 포함)로 줄여 다시 써라. 지금은 ${len}자다.`,
