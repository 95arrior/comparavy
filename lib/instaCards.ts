...
export async function articleToInstaCards(title: string, bodyHtml: string, keyword: string, userId?: string | null): Promise<InstaPack | null> {
  ...
  const res = await client.messages.create({
    ...
    messages: [{
      ...
      content: [
        ...
        "  character = ★주제를 의인화한 캐릭터의 영어 외형 묘사 2문장(2026-08-11 유저: '캐릭터까지 묘사해서 프롬프트에 녹여내자'): 종·형태·색·복장·표정 스타일을 구체적으로 — 이 묘사만 읽고 누가 그려도 같은 캐릭터가 나오게. ★몸 자체가 주제여야 한다(2026-08-12 유저: '부품주 얘긴데 캐릭터가 엉뚱하고 전 편과 옷까지 똑같다') — 몸 형태·색·복장을 키워드 소재에서 가져와라: 반도체 부품주=회로 무늬 칩 몸통+금색 핀+안전조끼, 청약=집 몸통, 대출=지갑 몸통, 성과급=월급봉투 몸통. generic 동전 몸통+남색 블레이저로 수렴 금지 — 소재가 다르면 몸과 복장이 반드시 달라야 한다. ★타겟 40~60대 문법(2026-08-11 유저 확정): 둥글둥글 단순한 국민 캐릭터 감성(카카오프렌즈 결)이되 ★반드시 3D 렌더 피규어 질감(2026-08-12 유저: '2D로 나오는데 입체감 있어야 해') — 'cute 3D rendered mascot, soft rounded volume like a vinyl toy figure, glossy eyes, Pixar-style render' 문구 필수, 'flat/2D illustration' 계열 단어 금지. 원색 고대비·디테일 최소, 표정은 크고 과장되게(멀리서도 읽히게), 복장에 주제와 어울리는 소품 딱 1개(공장·부품 소재=안전조끼, 은행·금융 소재=넥타이나 동그란 안경, 부동산=공인중개사 느낌 명찰 없는 조끼 — 편마다 달라지게). 금지: Z세대 밈·네온 코드, 로봇·외계인(이질감), 사기꾼풍 과장. 몸이나 배경에 글자·숫자 금지. 예(레버리지): 'A cute 3D rendered mascot, a round bouncy green coin with soft rounded volume like a vinyl toy figure, two bold red upward-arrow horns on its head, tiny navy suit vest, big glossy expressive eyes and a confident grin, Pixar-style render.'",
  ...
  const HARD_TERM_RE = /(청구권|처분|기각|소멸|재직|구성원|호황|슈퍼사이클|대용증권|이수번호|산정|귀속|경과조치|법인차량|수시납입|집행|출자|조합|수혜)/g;
    ...
    try {
      const fix = await client.messages.create({
        ...
        messages: [{ role: "user", content: [
          ...
          "규칙: 화자·반말·문체·구조(의인화 훅 '나 {키워드}인데!'→쉬운 사건 정리→열린 고리→'자세한 내용은 아래를 확인해!') 유지. ★이건 광고다 — 이유·결말·반전의 답을 새로 밝히지 마라(궁금증이 클릭 연료다). ★문장 7개 이하·숫자 2개까지 — 정보를 덜어내라. 공시·펀드 용어는 아예 빼라. ★강약 리듬 유지 — 3~7자 초단문 3개 이상, 반전 직전 '…' 뜸, 물음표 1개(같은 길이 문장 3연속 금지). 새 사실 금지 — 대사와 [본문]에 있는 것만. '블로그' 단어 금지. 대사 본문만 출력(따옴표·설명 없이).",
  ...
  try {
    ...
    if (pack.clip?.oneTake) pack.clip.oneTake = noBlog(easyWords(await fitOneTake(pack.clip.oneTake))).replace(/([!?\u2026])[.,]+/g, "$1").slice(0, 320); // 깨진 부호('!.', '!,') 청소 — 2026-08-12 실측
    return pack;
  } catch { return null; }
}
