...
export async function articleToInstaCards(title: string, bodyHtml: string, keyword: string, userId?: string | null): Promise<InstaPack | null> {
  ...
  const res = await client.messages.create({
    ...
    messages: [{
      ...
      content: [
        ...
        "  oneTake = ★15초 티저 광고 대본(2026-08-12 유저 확정: '15초짜리 광고라고 생각하고 짜야 해 — 너무 궁금해서 글로 읽어봐야 할 것 같게'): ★175~205자(초당 12.5자 × 약 14~16초). ★이건 요약이 아니라 광고다 — 목적은 정보 전달이 아니라 블로그 클릭이다. 구조: ①의인화 훅 — 반드시 '나 {키워드 핵심}인데!'로 시작(유저 실례: '나 삼성전자 투자 공시인데!') + 가장 충격적인 돈 팩트 1개 ②사건 초간단 정리 — 초등학생도 아는 쉬운 팩트 2~3개만, 어려운 세부·조건·숫자 계산은 전부 블로그로 미룬다 ③★열린 고리(핵심 심리 장치): 제일 궁금한 대목을 던지고 절대 풀지 마라 — 이유·결말·반전의 답을 대본에서 말하는 순간 클릭할 이유가 사라진다. ★티저는 막연한 여운('얼마나 붙을지는…')이 아니라 넘어가면 뭘 알게 되는지를 명사로 예고해라('진짜 돈 되는 얘기는 따로 있어 — 덕 보는 회사가 누군지', '언제 움직여야 하는지') — 궁금증은 구체적일수록 세다(2026-08-14 유저: '아직 유혹을 못 하고 있어'). ★단 예고한 답은 블로그 본문에 실제로 있어야 한다(유저: '허위사실은 절대 안 됨') — 본문 소제목들이 답하는 것 중에서만 예고를 골라라. 본문에 없는 정보 예고('수혜 회사가 누군지'인데 본문에 회사 목록이 없음)는 낚시라 절대 금지 ④CTA: 열린 고리와 길안내를 한 문장으로 — '어디를 언제 사야 하는지는… 화면 오른쪽 위 블로그 누르면 다 나와!' 결(★'아래' 안내 금지 — 클립 링크는 화면 오른쪽 위 블로그 버튼이다). ★훅 정합(2026-08-13 유저: '영상 보고 들어왔을 때 이탈하지 않게'): 첫 문장은 상단 고정 문구(topHook)가 건 약속을 즉시 이어받아라 — 들어온 사람이 3초 안에 '맞게 들어왔네'를 느껴야 한다, 훅과 다른 얘기로 시작하면 바로 나간다. ★비틀기(빠져들게): 문장마다 예상을 하나씩 배신해라 — '다들 A라고 보지? 틀렸어', 상식 뒤집기·엉뚱한 수혜자·의외의 숫자. 뻔한 다음 문장이 이탈 버튼이다. ★손해 FOMO(CTA 직전 필수, 유저: '자세히 안 보면 손해날 것 같은 어그로'): 안 보면 손해라는 감각 한 문장 — '모르고 지나가면… 남들 버는 거 구경만 하다 끝나', '이거 모르고 있다가 당하는 사람이 제일 많아' 결. 단 글에 있는 사실 기반 — 없는 손해 지어내기 금지. ★정보량 상한(2026-08-12 유저 실측: 문장 11개짜리는 '15초 동안 소화가 안 되고 잘 안 들린다'): 문장 7개 이하, 숫자 2개까지 — 팩트가 4개 넘으면 아무것도 안 들린다. ★산업 통계·전문 단위 금지(2026-08-14 유저: '57GW 같은 말은 사람들이 잘 몰라 — 생각하게 만드는 순간 영상이 훅 지나가고 유입이 죽는다'): GW·MW·성장률·생산능력·시장규모 예측치는 아예 빼라. 숫자는 돈(원)과 일상 숫자만. ★공시·펀드 용어(조합·수시납입·집행·출자·수혜 같은 것)는 번역도 말고 아예 빼라 — 그 설명이 곧 글의 몫이다. ★매수 타이밍·결론('~할 때 사야 돈 본다')도 답이다 — 말하는 순간 클릭 이유가 사라진다. 강약: 3~7자 초단문 2개 이상 + '…' 뜸 1개 이상 + 물음표 1개. ★긴급 속보 문체(2026-08-14 유저: '나근나근 금지 — 속보 전하듯'): 특종 물어온 사람처럼 느낌표 있는 문장을 절반 이상, 설명조('~입니다/~어요') 금지. ★TTS 발음 안전(유저 실측: '데이터센터'가 '데이터센트'로 읽힘): 영문 스펠링·약어 금지(한글 소리 표기로), 4음절 넘는 외래어 복합어는 띄어 쓰기('데이터 센터'), 문장 끝은 어미·조사로 확실히 닫기(외래어로 뚝 끝내면 끝음절이 뭉개진다). 반말·뒷담화 톤 유지.",
        "  acting = ★영상 연출 지시(2026-08-14 유저 2차: '가만히 서서 손만 움직이지 말고 대사 내용을 장면으로 창작' — 예: 인수 대사엔 유럽으로 점프해 계약서에 도장 찍는 장면): 영어 4~5문장. ★대사 비트마다 미니 장면을 만들어 순서대로 — 배경 전환('the scene shifts to a snowy European factory')·행동('stamps a giant blank contract with a confident thud')·복귀. 장면은 2~3개까지(많으면 캐릭터가 흔들린다), 시작과 끝은 스튜디오, 마지막은 화면 오른쪽 위를 가리키기. 캐릭터 디자인은 전 장면 동일 명시. 소품·배경 자유 — 단 글자·숫자가 보이는 것 금지(계약서는 백지, 간판은 무지, 차트 눈금 금지).",
  ...
  const HARD_TERM_RE = /(청구권|처분|기각|소멸|재직|구성원|호황|슈퍼사이클|대용증권|이수번호|산정|귀속|경과조치|법인차량|수시납입|집행|출자|조합|수혜|중복상장|재원|희석|신주|용처|재공시|생산능력|연평균|성장률|시장규모|GW|MW|TWh|기가와트)/g;
  ...
  // ★TTS 발음 안전화(2026-08-14 유저 실측: '데이터센터'를 '데이터센트'로 읽는 등 발음이 뭉개짐).
  //  원인 3종: 긴 외래어 복합어(띄면 안정)·영문 약어(제멋대로 읽음→한글 표기)·붙는 단위. 실물 잡힐 때마다 사전에 추가.
  const PRONOUNCE_MAP: [RegExp, string][] = [
    [/데이터센터/g, "데이터 센터"], [/블록체인/g, "블록 체인"], [/알고리즘/g, "알고리듬"],
    [/\bPF\b/g, "피에프"], [/\bETF\b/g, "이티에프"], [/\bISA\b/g, "아이에스에이"],
    [/\bIPO\b/g, "아이피오"], [/\bCEO\b/g, "씨이오"], [/\bHVAC\b/g, "공조 설비"], [/\bGPS\b/g, "지피에스"],
    [/\bIT\b/g, "아이티"], [/\bOTT\b/g, "오티티"],
  ];
  const pronounceFix = (t: string) => PRONOUNCE_MAP.reduce((acc, [re, to]) => acc.replace(re, to), t);
  ...
  try {
    ...
    const pack = {
      ...
      clip: segments.length >= 3 && hook.say ? { hook, segments, character, background: backgroundDesc, styleAnchor: anchor, basePrompt,
        ...
        videoPrompt: (() => {
          ...
          return `The character from the input image, unchanged in design and outfit. ${cut(acting, 600)} The character speaks in an urgent, fast-paced breaking-news tone — excited and punchy, never calm or soothing. Vertical 9:16, smooth cartoon motion, dynamic scene transitions following the dialogue beats, the character unchanged in design across every scene. Absolutely no captions, subtitles or overlay text anywhere — especially no Korean Hangul; contracts, signs and props stay blank. Lettering on the character's own logo body is fine. Natural mouth movement only.`.replace(/\s+/g, " ").trim();
        })(),
    ...
    if (pack.clip?.oneTake) pack.clip.oneTake = pronounceFix(fixCta(easyWords(await fitOneTake(pack.clip.oneTake)))).replace(/([!?\u2026])[.,]+/g, "$1").slice(0, 330); // noBlog 폐기(클립 CTA에 블로그 단어가 필요해짐) + 깨진 부호 청소
    return pack;
  } catch { return null; }
}
