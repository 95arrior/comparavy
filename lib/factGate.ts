...
// ★"2025년 9월 1일 이후부터 자동으로 1억 원 한도가 적용됩니다" — 이것도 정상이다(2026-08-02 실측 오탐).
//  '이후·이래'가 붙으면 그 날짜는 시작점이지 예고가 아니다. '현재·지금·이미'도 마찬가지.
const ALREADY_DONE = /(?:시행|적용|상향|인상|인하|변경|개편|도입)\s*(?:중|돼|되어|된|됐|했)|적용\s*중|시행\s*중|였습니다|왔습니다|올랐|내렸|바뀌었|이후|이래|현재|지금|이미/;
