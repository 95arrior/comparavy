...
const FINANCE_RULES: Rule[] = [
  // ★부정문 제외(2026-07-31 실측 — 자체 초안을 게이트에 돌리다 발견): "원금이 보장되지 않습니다"는
  //  우리가 반드시 써야 하는 고지 문장인데 high로 잡혔다. 자동발행 보류 배선 뒤라 올바른 고지가 발행을 막는다.
  //  게이트는 표기가 아니라 결과를 봐야 한다 — 같은 단어라도 부정하면 위반이 아니다(cardFinalGate 원칙과 같은 결).
  { test: /원금\s*(?:이|은|을)?\s*보장(?!\s*(?:되지|하지|받지|안)\s*)|원금\s*손실\s*(?:이|은)?\s*없(?!\s*다고)/g, type: "결과보장", law: FINANCE_LAW, reason: "원금 보장·무손실 단정은 투자권유 오인 표현입니다(예금자보호 제도의 공식 안내 인용 맥락만 예외).", severity: "high", suggestion: "원금 손실 가능성이 있는" },
