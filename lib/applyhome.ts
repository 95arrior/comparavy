...
function toSeed(r: Row, isRemainder: boolean): ApplyhomeSeed | null {
  ...
  return {
    ...
    newsContext: [
      ...
      // ★로또 각도(2026-08-07 유저 확정: "무조건 줍줍이나 진짜 시세차익 많이 낼 수 있는 비싼 동네여야만").
      //  이 카드가 보드에 섰다는 것 자체가 '전국이 쳐다보는 단지'라는 뜻이다(단지명 검색량 하한 통과).
      `★이 글의 각도는 '왜 이 청약이 로또로 불리는가'다: 검색자는 절차가 아니라 기회의 크기를 확인하러 온다.`,
      `★차익을 숫자로 단정하지 마라(분양가·시세 데이터가 이 카드에 없다). 대신 '차익을 스스로 확인하는 법'을 정확히 안내한다 — 공고문의 분양가 항목 위치, 국토부 실거래가 공개시스템에서 같은 단지·인근 단지 시세 찾는 순서.`,
      `★함께 반드시 다룰 것: 무순위(줍줍)라면 청약통장·거주지 요건이 어떻게 다른지, 전매 제한·실거주 의무가 있는지 — 이게 '로또'의 조건이자 함정이다.`,
    ].join("\n"),
  };
}
