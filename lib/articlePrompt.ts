...
// 전문업종 공통 SEO 콘텐츠 원칙 (general 제외 모든 업종에 주입). 홍보글이 아니라 '검색 사용자가 궁금해하는 정보글'.
const COMMON_SEO_PRINCIPLES = [
  ...
  "- 업체(병원·학원·사무소 등)는 정보 맥락 안에서 자연스럽게 1~2회만 언급한다. 넣을 자리가 없으면 넣지 않는다. 글 전체를 업체 소개로 만들지 않는다.",
...
export function buildSystemPrompt(vertical?: string): string {
  ...
  // general(또는 미지정)은 기존 동작 100% 유지. 전문업종(medical/academy/professional/b2b)만 공통 SEO 원칙 + 업종별 지침을 덧붙인다.
  if (vertical && vertical !== "general" && VERTICAL_SYSTEM[vertical]) {
