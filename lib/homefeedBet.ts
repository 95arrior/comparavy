  ...
  try {
    ...
    const res = await client.messages.create({
      ...
      messages: [{
        role: "user",
        content: [
          `네이버 홈피드(홈판)에서 폭발적 반응을 노리는 경제 블로그 글감 1개를 만든다. 검색 SEO용이 아니다 — 스크롤을 멈추게 하는 호기심·반전이 전부다.`,
          ...
          `제목 규격: 낚시 아닌 훅 — 본문이 100% 이행할 약속만. 숫자·반전·1인칭 관찰 중 2개 이상 결합(예: "30대 평균 저축액, 통계 까보고 좀 놀랐습니다"). 물음표 남발·과장(충격·경악) 금지.`,
          `JSON만 출력: {"keyword":"주제 앵커(15자 이내)","title":"훅 제목(32자 이내)","thumbCopy":"썸네일 문구(12자 이내)","angle":"본문이 다룰 핵심 각도 2문장"}`,
        ].join("\n"),
      }],
    });
