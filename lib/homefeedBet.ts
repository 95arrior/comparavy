...
async function liveSeedBlock(sub: string, exclude: Set<string>): Promise<string | null> {
  try {
    ...
    return [
      `★[오늘 수확한 실제 이슈 — ★반드시 이 안에서 소재를 고른다] 공고·공시·보도자료·뉴스에서 실제로 확인된 것들이다.\n★백지에서 주제를 떠올리지 마라: 이 블로그는 이미 100편 넘게 썼고, 떠올린 주제는 거의 예외 없이 이미 쓴 것과 겹쳐 버려진다.\n★아래 목록에 마땅한 게 없으면 그 유형은 포기하고 card를 null로 둔다 — 억지로 만든 카드는 어차피 중복으로 죽는다.`,
    ...
    const res = await client.messages.create({
      ...
      messages: [{
        ...
        content: [
          ...
          `절대 원칙: ①거짓 사연·지어낸 경험 금지 — 공식 통계·실제 제도·계산으로만 성립하는 주제 ②전 국민 이해관계(대상이 넓을수록 좋다) ③이미 쓴 주제 제외: ${[...usedKeywords].slice(0, 120).join(", ") || "(없음)"}`,
